...
def rescale_image(image, hash_size=8):
    image = image.resize((hash_size, hash_size), Image.ANTIALIAS)
    image.save('rescale.jpg')


def rescale_and_convert_image(image, hash_size=8):
    image = image.convert("L").resize((hash_size, hash_size), Image.ANTIALIAS)
    image.save('rescaleconvert.jpg')


def rescale_and_convert_and_show_mean_image(image, hash_size=8):
    image = image.convert("L").resize((hash_size, hash_size), Image.ANTIALIAS)
    # find average pixel value; 'pixels' is an array of the pixel values, ranging from 0 (black) to 255 (white)
    pixels = numpy.asarray(image)
    avg = pixels.mean()
    ...
    scale_size = 240
    image_new = image.resize((scale_size, scale_size), Image.NONE)
    ...
    # for r, row in enumerate(pixels):
        for c, col in enumerate(row):
            ...
            txt = "1" if e else "0"
            # txt = str(e)
            draw.text((c * factor + offset, r * factor + offset), txt)
            # draw.text((r * factor + offset, c * factor + offset), txt)

    image_new.save('rescaleconvertmean.jpg')
...
if __name__ == '__main__':
    image_path = '/Users/tguehring/Projects/master_thesis/thesis/figures/Twitter1.jpg'
    img = Image.open(image_path)
    # rescale_image(img)
    # rescale_and_convert_image(img)
    rescale_and_convert_and_show_mean_image(img)
